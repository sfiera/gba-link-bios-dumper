...
bool confirm_handshake_data(u8 client_mask, const u8* client_data, u8* handshake_data) {
...
bool detect_clients(u16 cmd, u16 ack, u8* client_mask) {
...
bool confirm_clients(u16 cmd, u16 ack, u8 client_mask) {
...
bool send_header(u8 client_mask, const u8* rom) {
...
bool send_palette(u8 client_mask, u8* client_data) {
...
bool sendRom(const u8* rom, u32 romSize) {
    while (true) {
        ...
        if (detect_clients(CMD_HANDSHAKE, ACK_HANDSHAKE, &param.client_bit) &&
            confirm_clients(CMD_CONFIRM_CLIENTS, ACK_HANDSHAKE, param.client_bit) &&
